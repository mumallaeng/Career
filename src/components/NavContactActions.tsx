...
export default function NavContactActions() {
  ...
  const tooltipText = (target: CopyTarget, value: string) =>
    copiedTarget === target ? `${copy.aria.copied}: ${value}` : value;

  return (
    <>
      <button
        type="button"
        className={`navbar-contact-action${copiedTarget === 'phone' ? ' is-copied' : ''}`}
        onClick={() => void handleCopy('phone', contactPhone)}
        aria-label={`${copy.aria.copyPhone}: ${contactPhone}`}
        aria-describedby={phoneTooltipId}
      >
        <svg className="navbar-contact-icon" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        ...
        <span id={phoneTooltipId} className="navbar-contact-tooltip" role="tooltip">
          {tooltipText('phone', contactPhone)}
        </span>
      </button>

      <button
        type="button"
        className={`navbar-contact-action${copiedTarget === 'email' ? ' is-copied' : ''}`}
        onClick={() => void handleCopy('email', contactEmail)}
        aria-label={`${copy.aria.copyEmail}: ${contactEmail}`}
        aria-describedby={emailTooltipId}
      >
        <svg
        ...
        <span id={emailTooltipId} className="navbar-contact-tooltip" role="tooltip">
          {tooltipText('email', contactEmail)}
        </span>
      </button>
      ...
      <Link
        ...
        aria-label={copy.aria.visitGithub}
